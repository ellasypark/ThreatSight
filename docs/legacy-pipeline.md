# Original access-log pipeline

> This documents the existing research pipeline. For the new default local service workflow and optional dependency installation, start with the [current README](../README.md). Legacy numbers below were inherited and are not new service-investigation measurements.
...
<p align="center">
  <img src="dashboard.jpg"  alt="ThreatSight dashboard top">
  <img src="dashboard1.jpg" alt="ThreatSight dashboard bottom">
</p>
...
> If your network intercepts HTTPS and Anthropic calls fail with a certificate error, set `SSL_CERT_FILE` to your trusted corporate CA bundle. Certificate verification stays enabled.
...
- **The LLM layers cost money and add latency.** The deterministic detection and evaluation paths run without an API key; the LLM layer requires an API key.
